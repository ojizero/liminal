...
# Allowed types per the Angular convention (Conventional Commits 1.0.0).
...
cc_allowed_types_csv() {
  ...
  for type in "${CC_ALLOWED_TYPES[@]}"; do
    if [[ "$first" == true ]]; then
      first=false
      printf '%s' "$type"
    else
...
cc_type_allowed() {
  ...
  for allowed in "${CC_ALLOWED_TYPES[@]}"; do
...
cc_parse_subject() {
  ...
  if ! cc_type_allowed "$CC_TYPE"; then
    return 1
  fi
